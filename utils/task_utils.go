...
import (
	...
	apiclient "github.com/arcfraofficial/aoc-go-sdk/client"
...
func WaitTask(ctx context.Context, client *apiclient.Aoc, id *string, interval time.Duration) error {
...
func WaitTasks(ctx context.Context, client *apiclient.Aoc, ids []string, interval time.Duration) error {
	if interval < 1*time.Second {
		interval = 1 * time.Second
	}
	errorMsgs := ""
